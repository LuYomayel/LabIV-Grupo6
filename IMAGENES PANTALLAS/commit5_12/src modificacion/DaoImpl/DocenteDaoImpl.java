...
public class DocenteDaoImpl implements DocenteDao{
	...
	public int agregarDocente(Docente docente) {
		...
		String query="INSERT INTO docentes(legajo_Docente, Dni_Docente, Nombre_Docente, Apellido_Docente, FechaNac_Docente, Direccion_Docente, Localidad_Docente, Nacionalidad_Docente, Email_Docente, Telefono_Docente, idCarrera)VALUES("+docente.getLegajo()+",'"+docente.getDni()+"','"+docente.getNombre()+"','"+docente.getApellido()+"','"+docente.getFechanacimiento()+"','"+docente.getDireccion()+"','"+docente.getLocalidad()+"', '"+docente.getNacionalidad()+"', '"+docente.getEmail()+"' , '"+docente.getTelefono()+"','"+docente.getIdCarrera()+"')";
		Connection cn = null;
		int filas =0;
		try {
	...
	public int eliminarDocente(int legajo) {
		...
		try
		{
			cn = DriverManager.getConnection(host+dbName, user,pass);
			Statement st = cn.createStatement();
			String query = "DELETE FROM docentes WHERE legajo="+legajo;
			filas=st.executeUpdate(query);
		}
		catch(Exception e)
		{
	...
	public Docente obtenerDocente(int legajo) {
		...
		try {
			cn = DriverManager.getConnection(host+dbName,user,pass);
			Statement st =cn.createStatement();
			String query = "SELECT * FROM docentes WHERE Legajo="+legajo;
			ResultSet rs = st.executeQuery(query);
			rs.next();
			x.setLegajo(rs.getInt("legajo"));
			x.setDni(rs.getString("dni"));
